...
async function handleAppSubscriptionUpdate(store, payload) {
  ...
  // Shopify tells us the moment a subscription is approved, declined,
  // cancelled, expired or frozen. Reacting here keeps the local plan honest
  // without waiting for the merchant to open the app — otherwise a cancelled
  // subscription would keep its limits until someone happened to visit.
  return prisma.store.update({
    where: { id: store.id },
    data: {
      plan: active ? planIdFromSubscriptionName(subscription.name) : 'FREE',
