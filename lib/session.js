...
export async function getCurrentStore({ autoInstall = true } = {}) {
  ...
  // finalizeInstall is a no-op once its markers are set, so this costs two
  // field reads on the normal path and repairs a store whose profile sync or
  // webhook registration did not complete on a previous visit.
  if (store && !store.uninstalledAt) return finalizeInstall(store);
  ...
  const installed = await completeManagedInstall(verifiedShop, idToken);
  return installed ? finalizeInstall(installed) : null;
}
