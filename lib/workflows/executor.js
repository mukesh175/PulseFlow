...
import { renderBody } from '@/lib/workflows/merge';
...
async function sendEmailStep({ enrollment, step, stepIndex, now, channels }) {
  ...
  // A discount created earlier in this journey is what {{discount_code}} refers
  // to. Looked up per send rather than carried along, so a step added between
  // the discount and the email still resolves.
  const grant = await prisma.discountGrant.findFirst({
    where: { enrollmentId: enrollment.id, revokedAt: null },
    orderBy: { createdAt: 'desc' },
  });

  // The customer's name lives on the mirrored order, not on the enrollment —
  // and only looked up when the body actually asks for it, so the common case
  // costs nothing.
  let customerName = null;
  if (`${step.subject}${step.body}`.includes('customer_name')) {
    const order = await prisma.order.findFirst({
      where: { shopId: enrollment.shopId, customerEmail: enrollment.customerEmail, customerName: { not: null } },
  ...
  const values = {
    discount_code: grant?.code ?? null,
    customer_name: customerName,
    store_name: enrollment.store.shopName || enrollment.store.shopDomain,
  };

  const subject = renderBody(step.subject, values);
  const body = renderBody(step.body, values);
  const missing = [...new Set([...subject.missing, ...body.missing])];

  // A message that would go out reading "Here is {{discount_code}}" is worse
  // than one that does not go out. Skipped, with the reason recorded, so the
  // merchant can see what happened rather than hearing it from a customer.
  if (missing.length) {
    await prisma.messageLog.update({
      where: { id: row.id },
      data: { status: 'SKIPPED', skipReason: `missing_placeholder:${missing.join(',')}` },
    });
    return {
      kind: 'continue',
      status: 'SUCCEEDED',
      result: { messageLogId: row.id, status: 'SKIPPED', skipReason: `missing_placeholder:${missing.join(',')}` },
    };
  }
...
/**
 * Update the enrollment and keep the relations the caller loaded.
 *
 * `prisma.enrollment.update` returns the row without `store` or `snapshot`, so
 * assigning its result over `current` would silently drop them — and the next
 * step to read `enrollment.store` would fail. The dry-run channel ignores the
 * store, so this stayed invisible until a real channel needed `store.id`.
 */
async function transition(enrollment, data) {
