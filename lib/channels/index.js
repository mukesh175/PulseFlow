...
import { createShopifyDiscountChannel } from '@/lib/channels/discount';
...
export function createDryRunChannels({ onEvent } = {}) {
  ...
  return {
    ...
    async sendEmail({ store, recipient, subject, enrollmentId }) {
      // storeId is recorded so a harness can prove the store relation survived
      // the journey — the dry-run channel has no other use for it.
      record({ type: 'email', storeId: store?.id ?? null, recipient, subject, enrollmentId });
      return {
        status: 'SKIPPED',
        skipReason: 'dry_run',
    ...
    async createDiscount({ store, percentage, amount, expiresAt, enrollmentId }) {
      ...
      record({ type: 'discount', storeId: store?.id ?? null, code, percentage, amount, expiresAt, enrollmentId });
...
/**
 * The channel set the scheduler actually uses. Both halves are live.
 *
 * `live` is kept rather than removed now that it reads all true: it is what
 * `unsupportedSteps` consults, and a future channel — WhatsApp, which the brief
 * rules out of v1 — arrives as another false here rather than as a new
 * mechanism.
 */
export function createChannels() {
  const email = createEmailChannel();
  const discount = createShopifyDiscountChannel();

  return {
    sendEmail: email.sendEmail,
    createDiscount: discount.createDiscount,
    live: { email: true, discount: true },
  };
}
